    ...
    public static unsafe class LZO
    {
        ...
        public static byte[] Decompress(byte[] data, int decompressedSize)
        {
            ...
            fixed (byte* a = &data[0])
            {
                ...
                {
                    var result = (NativeMethods.LZOkayResult)NativeMethods.LZOkayDecompress(
                        b,
                        ref rawBufSize,
                        a,
                        data.Length);

                    if (result != NativeMethods.LZOkayResult.Success)
        ...
        public static byte[] Compress(byte[] data)
        {
            ...
            fixed (byte* a = &data[0])
            {
                ...
                fixed (byte* b = &temp[0])
                {
                    var result = (NativeMethods.LZOkayResult)NativeMethods.LZOkayCompress(
                        b,
                        ref outputSize,
                        a,
                        data.Length);
