using PhilLibX.Media3D.Cast;
using PhilLibX.Media3D.CoDXAsset;
using PhilLibX.Media3D.SEAnim;
using PhilLibX.Media3D.SEModel;
using System;
...
namespace PhilLibX.Media3D.Translator
{
    ...
    public class Graphics3DTranslatorFactory
    {
        ...
        public Graphics3DTranslatorFactory WithDefaultTranslators()
        {
            ...
            Translators.Add(new CastModelTranslator());
        ...
        {
            if (TryGetTranslator(null, filePath, out var translator) && translator.SupportsWriting)
            {
        ...
        public T Load<T>(Stream stream, string name) where T : Graphics3DObject
        {
            ...
            var result = io.Objects.FirstOrDefault(x => x.GetType() == type) ?? throw new Empty3DFileException();
