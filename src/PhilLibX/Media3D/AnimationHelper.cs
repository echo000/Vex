using PhilLibX.Media3D.AnimationSampling;
using System;
...
namespace PhilLibX.Media3D
{
    public class AnimationHelper
    {
        ...
        /// </summary>
        public static (int, int) GetFramePairIndex<T>(List<AnimationFrame<T>>? list,
                                                      float time, float startTime,
        ...
        public static float GetWeight(List<AnimationFrame<float>> weights, float time,
                                      float startTime, float defaultWeight,
                                      ref int cursor)
        {
            var (firstIndex, secondIndex) =
                GetFramePairIndex(weights, time, startTime, cursor: cursor);
            ...
            if (firstIndex != -1)
            {
                ...
                else
                {
                    ...
                    var lerpAmount =
                        (time - (startTime + firstFrame.Time)) /
                        ((startTime + secondFrame.Time) - (startTime + firstFrame.Time));

                    result = (firstFrame.Value * (1 - lerpAmount)) +
                             (secondFrame.Value * lerpAmount);
                }
        ...
        /// </summary>
        /// <param name="root">The animation that the input animation is being appened
        /// onto.</param> <param name="input">The animation being appened.</param>
                ...
                foreach (var target in input.SkeletonAnimation.Targets)
                {
                    ...
                    if (target.TranslationFrames is not null)
                    {
                        ...
                        foreach (var frame in target.TranslationFrames)
                            newTarget.TranslationFrames.Add(
                                new(start + frame.Time, frame.Value));
                    }
        ...
        /// <summary>
        /// Converts the provided <see cref="Animation"/> that contains a <see
        /// cref="SkeletonAnimation"/> to the given transform space.
        /// </summary>
        /// <param name="input">The input <see cref="Animation"/>. If the provided
        /// input has no skeleton animation, then this is returned.</param> <param
        /// name="newSpace">The new <see cref="TransformSpace"/> to convert the
        /// animation to.</param> <returns>Resulting animation, if no skeleton
        /// animation is provided or the space is the same, the input animation is
        /// returned.</returns>
        public static Animation ConvertTransformSpace(Animation input,
        ...
        /// <param name="input">The input <see cref="Animation"/>. If the provided
        /// input has no skeleton animation, then this is returned.</param> <param
        /// name="skeleton">The <see cref="Skeleton"/> to use for sampling, if null,
        /// it is taken from the <see cref="SkeletonAnimation"/>.</param> <param
        /// name="newSpace">The new <see cref="TransformSpace"/> to convert the
        /// animation to.</param> <returns>Resulting animation, if no skeleton
        /// animation is provided or the space is the same, the input animation is
        /// returned.</returns> <exception cref="ArgumentNullException">Thrown if no
        /// skeleton is provided.</exception>
        public static Animation ConvertTransformSpace(Animation input,
                                                      Skeleton? skeleton,
                                                      TransformSpace newSpace)
        {
            // If no skeleton animation is provided, then we can do early quit, and
            // return this.
            if (input.SkeletonAnimation == null)
                return input;
            ...
            for (int i = 0; i < sampler.FrameCount; i++)
            {
                ...
                foreach (var target in sampler.SkeletonAnimationSampler!.TargetSamplers)
                {
                    if (target.Value.Target != null)
                    {
                        ...
                        {
                            newTarget.AddTranslationFrame(
                                i, target.Value.Bone.CurrentLocalTranslation);
                            newTarget.AddRotationFrame(i,
                                                       target.Value.Bone.CurrentLocalRotation);
                        }
                        else
                        {
                            newTarget.AddTranslationFrame(
                                i, target.Value.Bone.CurrentWorldTranslation);
                            newTarget.AddRotationFrame(i,
                                                       target.Value.Bone.CurrentWorldRotation);
                        }
                    }
                }
            }
