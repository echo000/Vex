...
namespace PhilLibX.Media3D.SEAnim
{
    /// <summary>
    ...
    public sealed class SEAnimTranslator : Graphics3DTranslator
    {
        ...
        public override void Write(Stream stream, string filePath, Graphics3DTranslatorIO input)
        {
                ...
                foreach (var bone in data.SkeletonAnimation.Targets)
                {
                    if (bone.TransformType != TransformType.Parent && bone.TransformType != animationType)
                    {
                        ...
                        switch (bone.TransformType)
                        {
                            ...
                            case TransformType.Delta: boneModifiers[index] = 3; break;
                        }
                    }
            ...
            switch (transformType)
            {
                ...
                case TransformType.Relative: writer.Write((byte)2); break;
                case TransformType.Delta: writer.Write((byte)3); break;
                default: writer.Write((byte)0); break;
            }

            writer.Write(data != null ? data.Looping ? (1 << 0) : 0 : 0);
            ...
            if (data?.SkeletonAnimation != null)
            {
                ...
                {
                    writer.WriteNullTerminatedString(bone.BoneName.Replace('.', '_'));
                }
            ...
            if (data?.MorphAnimation != null)
            {
                writer.Write(0xA646E656C424553);
                writer.Write(data.MorphTargetCount);

                foreach (var morph in data.MorphAnimation.Targets)
                {
                    writer.WriteNullTerminatedString(morph.MorphName);
                }
